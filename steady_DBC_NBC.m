...
nodesDir0 = nodes_xL';                         % Nodes at Dirichlet BC: p=0

nodes_nbc = [nodes_y0; nodes_yW];
...
%     [K,f] = old_FEM_matrices_steady(X,T,referenceElement,Kv,p);
    [K,f] = FEM_matrices_steady(X,T,referenceElement,Kv,p,NBC);
...
xlabel('x-axis (m)')
ylabel('Pressure (kPa)')
title('Pressure profile at bottom boundary')
set(gca, 'FontSize',14);
grid on;
...
xlabel('x-axis (m)')
ylabel('Pressure (kPa)')
title('Pressure profile at top boundary')
set(gca, 'FontSize',14);
grid on;
